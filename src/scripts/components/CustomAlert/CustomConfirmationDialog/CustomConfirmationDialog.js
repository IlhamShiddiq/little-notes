...
const CustomConfirmationDialog = ({locale = 'en', message, onClose, onClickAccepted}) => {
    return (
        <div className='confirmation-alert'>
            <h1>{message}</h1>
            <button className='confirmation-alert__reject-button' onClick={onClose}>
                {ConfirmationButton[locale].reject_button}
            </button>
            <button className='confirmation-alert__accept-button' onClick={onClickAccepted}>
